        ...
        <section className="py-16 md:py-24 bg-card">
          <div className="container mx-auto px-4 max-w-4xl">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {[
                { icon: TrendingDown, text: "Tired of not knowing if you're getting the best price?" },
                { icon: Globe, text: "Wasting time checking multiple websites and WhatsApp groups?" },
                { icon: ShieldCheck, text: "Buying from unverified sellers and getting scammed?" },
              ].map(({ icon: Icon, text }, i) => (
                <div key={i} className="flex flex-col items-center text-center gap-4 p-6 rounded-2xl border border-border bg-background shadow-sm">
                  <div className="w-12 h-12 rounded-full bg-destructive/10 flex items-center justify-center">
                    <Icon className="w-6 h-6 text-destructive" />
                  </div>
                  <p className="text-foreground text-base md:text-lg leading-relaxed">{text}</p>
                </div>
              ))}
            </div>
          </div>
                ...
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-background border border-border">
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`font-heading font-bold text-lg ${i === 1 ? "text-primary" : "text-foreground"}`}>
                        {i === 1 ? "GHS 2,499" : i === 2 ? "GHS 2,750" : "GHS 3,100"}
                      </div>
                      {i === 1 && (
                        <span className="text-xs font-semibold bg-primary/10 text-primary px-2 py-1 rounded-full">Best Price</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <section className="py-12 bg-card border-y border-border">
          <div className="container mx-auto px-4 max-w-3xl">
            <div className="grid grid-cols-3 gap-6 text-center">
              <div>
                <p className="font-heading text-3xl md:text-4xl font-bold text-primary">{vendorCount ?? "—"}</p>
                <p className="text-sm text-muted-foreground mt-1">Verified Vendors</p>
              </div>
              <div>
                <p className="font-heading text-3xl md:text-4xl font-bold text-primary">{countryCount ?? "—"}</p>
                <p className="text-sm text-muted-foreground mt-1">Countries</p>
              </div>
              <div>
                <p className="font-heading text-3xl md:text-4xl font-bold text-primary">{userCount ?? "—"}</p>
                <p className="text-sm text-muted-foreground mt-1">Smart Buyers</p>
              </div>
            </div>
            ...
            <div className="grid gap-8 md:grid-cols-3">
              {[
                ...
                <div key={i} className="flex flex-col items-center text-center gap-4">
                  ...
                  <div className="font-heading text-sm font-semibold text-muted-foreground tracking-widest uppercase">
                    Step {i + 1}
                  </div>
